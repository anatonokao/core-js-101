...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    const regexp = new RegExp(arrStr[i], 'g');
    if ([...str.matchAll(regexp)].length === 1) return arrStr[i];
  }
  return null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${firstBracket}${numbers}${lastBracket}`;
}
...
function isCreditCardNumber(ccn) {
  let numbers = String(ccn).split('').reverse();
  numbers = numbers.map((num, index) => {
    if (index % 2) {
      if (num * 2 > 9) return num * 2 - 9;
      return num * 2;
    }
    return num;
  });
  const sum = numbers.reduce((acc, num) => {
    const sumAcc = acc + Number(num);
    return sumAcc;
  }, 0);
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  function recursion(numbers) {
    res = String(numbers).split('').reduce((acc, number) => acc + Number(number), 0);
    if (Number(res) > 9) recursion(res);
    return res;
  }
...
function getCommonDirectoryPath(pathes) {
  const res = [];
  for (let i = 0; i < pathes[0].split('/').length; i += 1) {
    let matchCounter = 0;
    for (let j = 0; j < pathes.length; j += 1) {
      if (pathes[0].split('/')[i] === pathes[j].split('/')[i]) matchCounter += 1;
    }
    if (matchCounter === pathes.length) res.push(pathes[0].split('/')[i]);
  }
